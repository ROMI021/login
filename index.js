...
prenameInput.addEventListener('input', () => validatePrename());
...
// Note: Logic kept identical, just works with new CSS positioning
...
function validateName() {
    ...
    const feedback = nameInput.closest('.input-group').querySelector('.input-feedback');

    if (value === '') {
        resetFeedback(nameInput, feedback);
    } else if (value.length < 2) {
        setError(nameInput, feedback, 'Minimum 2 caractères');
    } else {
        setSuccess(nameInput, feedback, 'Valide');
    }
    checkFormValidity();
}

function validatePrename() {
    const value = prenameInput.value.trim();
    const feedback = prenameInput.closest('.input-group').querySelector('.input-feedback');

    if (value === '') {
        resetFeedback(prenameInput, feedback);
    } else if (value.length < 2) {
        setError(prenameInput, feedback, 'Minimum 2 caractères');
    } else {
        setSuccess(prenameInput, feedback, 'Valide');
    }
    checkFormValidity();
}
...
function validatePassword() {
    ...
    const strengthBar = passwordInput.closest('.input-group').querySelector('.strength-bar');
    ...
    if (value === '') {
        ...
        setError(passwordInput, feedback, 'Minimum 8 caractères');
    } else if (!hasUpperCase || !hasLowerCase || !hasNumber || !hasSpecialChar) {
        setError(passwordInput, feedback, 'Majuscule, minuscule, chiffre et caractère spécial requis');
    } else {
        setSuccess(passwordInput, feedback, 'Mot de passe fort');
    }
    checkFormValidity();
}

/* ============================================
   FONCTIONS D'AIDE - FEEDBACK (UPDATED)
   ============================================ */
...
function checkFormValidity() {
    ...
    isFormValid = isNameValid && isMailValid && isPasswordValid;
...
form.addEventListener('submit', (e) => {
    ...
    if (isFormValid) {
        ...
        btnText.textContent = 'Succès !';
        submitBtn.style.background = 'var(--success)';

        setTimeout(() => {
            alert('🎉 Bienvenue dans le futur.');
